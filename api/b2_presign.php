 ...
 * Authenticates via API token, returns a single S3-compatible presigned PUT URL
 * so the caller can upload a file directly to Backblaze B2.
 *
 * Request (POST or GET):
...
$p = array_merge($_GET, $_POST); // POST takes precedence
...
// Generate UUID v4 if caller did not supply one
if ($uuid === '') {
    $uuid = sprintf(
...
$datetime   = date('Ymd_His');   // e.g. 20260423_153045
